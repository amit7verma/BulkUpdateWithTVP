...
[Created] DATETIME CONSTRAINT DF_PERSON_CREATED DEFAULT(GETDATE()),
...
,EnrollmentDate	datetime
,Discriminator	nvarchar(50)
,DataVersion binary(100) null
,Created DateTime
,Updated DateTime
,DataState varchar(10)
)
go
if object_id('dbo.BulkUpdatePerson') is not null
	drop proc dbo.BulkUpdatePerson
go
Create Proc dbo.BulkUpdatePerson
@inputData dbo.PersonType Readonly
as
Begin
	set nocount off
	...
	set 
		...
		,p.EnrollmentDate=d.EnrollmentDate
		,p.Discriminator=d.Discriminator
		,P.Updated = Getdate()
	from dbo.Person p
	join @inputData d on p.PersonID = d.PersonID and p.DataVersion=d.DataVersion
	where d.DataState='Modified'

	Delete p
	from dbo.Person p
	join @inputData d on p.PersonID = d.PersonID and p.DataVersion=d.DataVersion
	where d.DataState='Deleted'

	declare @personIds table(pId int not null)
	...
	output inserted.PersonId into @personIds
	select 
		LastName
		,FirstName
		,HireDate
	...
	and d.DataState='Added'

	select * from Person where PersonID in (select pId from @personIds)
